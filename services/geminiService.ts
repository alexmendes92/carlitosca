...
import { PostState, GeneratedPostContent, ArticleState, GeneratedArticle, InfographicState, GeneratedInfographic, ConversionState, ConversionResult, PostFormat, MessageTemplateState, TrendTopic } from "../types";
...
export const generatePostText = async (state: PostState): Promise<GeneratedPostContent> => {
  ...
  if (state.uploadedImage) {
      ...
      prompt = `
        ...
        1. Analise a patologia. Identifique a dor do paciente.
        2. Identifique um mito ou erro comum.
        3. Estruture: Gancho (Headline), Corpo Educativo (analogias), CTA (Ético: "Agende sua avaliação" ou "Tire suas dúvidas").
...
// NEW: GENERATE NPS REQUEST
export const generateNPSMessage = async (patientName: string): Promise<string> => {
    checkApiKey();
    const prompt = `
        Escreva uma mensagem curta de WhatsApp pedindo avaliação no Google.
        Paciente: ${patientName}
        
        ${CFM_COMPLIANCE_INSTRUCTIONS}
        
        A mensagem deve ser ética, sem oferecer vantagem em troca da avaliação.
    `;
    ...
    return response.text || "Olá! Gostaríamos de saber como foi sua experiência. Poderia nos avaliar no Google? [LINK]";
};
