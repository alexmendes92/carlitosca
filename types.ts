...
export interface Appointment {
  ...
  // New Fields for Smart Agenda
  financialStatus?: FinancialStatus;
  value?: number;
  tags?: string[]; // "VIP", "Atleta", "Prioridade"
  surgeryChecklist?: {
      fasting: boolean; // Jejum
      exams: boolean; // Exames
      materials: boolean; // Materiais/OPME
      anesthetist: boolean; // Anestesista
  };
  voiceNotes?: string[]; // URLs or text transcription
}
...
export interface RTSMetrics {
    patientName: string;
    limbSymmetry: number; // 0-100%
    painScore: number; // 0-10 (Visual Analog)
    romExtension: number; // degrees
    romFlexion: number; // degrees
    hopTest: number; // 0-100%
    psychologicalReadiness: number; // 0-100 (ACL-RSI)
}
